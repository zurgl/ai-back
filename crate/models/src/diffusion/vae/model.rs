...
impl AutoEncoderKL {
    pub fn new(config: AutoEncoderKLConfig, device: tch::Device) -> Self {
        let mut vs_ae = VarStore::new(device);
        let vs = vs_ae.root();
        ...
        let weights = path::weights(Engine::VAEDecoder);
        vs_ae.load(weights).ok().unwrap();
